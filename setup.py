...
class CreateData(Command):
  ...
  def run(self):
    ...
    try:
      ...
      rmtree(os.path.join(here, 'cigar_aligner/data'))
    except OSError:
      pass

    # Generate data tables for main specification
    req_dir = os.path.join(here, 'cigar_aligner/data/main_spec')
    ...
    req_dir = os.path.join(here, 'cigar_aligner/data/bells_and_whistles')
    os.system(f'mkdir -p {req_dir}')
    with open(os.path.join(req_dir,'input_01.tsv'), 'w') as f:
      writer = csv.writer(f, delimiter='\t', quotechar='"')
      writer.writerow(['TR1', 'CHR1', '3', '8M7D6M2I2M11D7M'])
